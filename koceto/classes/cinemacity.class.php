...
/**
 * PHP CINEMACITY.com Grabber
 *
 * This PHP library enables you to scrap data from CINEMACITY.com.
 *
 *
 ...
 * @link    https://github.com/FabianBeiner/PHP-CINEMACITY-Grabber GitHub Repository
 * @version 6.0.4
 */
class CINEMACITY
{
     ...
     */
    const CINEMACITY_DEBUG = false;

    /**
     * Set the preferred language for the User Agent.
     */
    const CINEMACITY_LANG = 'bg, bg-BG;q=0.8';

    /**
     * Define the timeout for cURL requests.
     */
    const CINEMACITY_TIMEOUT = 15;
     ...
     */
    const CINEMACITY_AKA           = '~<h5>Also Known As:<\/h5>(?:\s*)<div class="info-content">(?:\s*)"(.*)"~Ui';
    const CINEMACITY_ASPECT_RATIO  = '~<h5>Aspect Ratio:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_AWARDS        = '~<h5>Awards:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_CAST          = '~<td class="nm"><a href="\/name\/(.*)\/"(?:.*)>(.*)<\/a><\/td>~Ui';
    const CINEMACITY_CERTIFICATION = '~<h5>Certification:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_CHAR          = '~<td class="char">(.*)<\/td>~Ui';
    const CINEMACITY_COLOR         = '~<h5>Color:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_COMPANY       = '~<h5>Company:<\/h5>(?:\s*)<div class="info-content"><a href="\/company\/(.*)\/">(.*)</a>(?:.*)<\/div>~Ui';
    const CINEMACITY_COUNTRY       = '~<a href="/country/(\w+)">(.*)</a>~Ui';
    const CINEMACITY_CREATOR       = '~<h5>(?:Creator|Creators):<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_DIRECTOR      = '~<h5>(?:Director|Directors):<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_GENRE         = '~<a href="\/Sections\/Genres\/([a-zA-Z_][\w-]*+)\/">(.*)<\/a>~Ui';
    const CINEMACITY_ID            = '~((?:tt\d{6,})|(?:itle\?\d{6,}))~';
    const CINEMACITY_LANGUAGE      = '~<a href="\/language\/(\w+)">(.*)<\/a>~Ui';
    const CINEMACITY_LOCATION      = '~href="\/search\/title\?locations=(.*)">(.*)<\/a>~Ui';
    const CINEMACITY_MPAA          = '~<h5><a href="\/mpaa">MPAA<\/a>:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_NAME          = '~href="\/name\/(.*)\/"(?:.*)>(.*)<\/a>~Ui';
    const CINEMACITY_NOT_FOUND     = '~<h1 class="findHeader">No results found for ~Ui';
    const CINEMACITY_PLOT          = '~<h5>Plot:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_PLOT_KEYWORDS = '~<h5>Plot Keywords:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_POSTER        = '~<link rel="image_src" href="(.*)">~Ui';
    const CINEMACITY_RATING        = '~<div class="starbar-meta">(?:\s*)<b>(.*)\/10<\/b>~Ui';
    const CINEMACITY_RELEASE_DATE  = '~<h5>Release Date:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_RUNTIME       = '~<h5>Runtime:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_SEARCH        = '~<td class="result_text"> <a href="\/title\/(tt\d{6,})\/(?:.*)"(?:\s*)>(?:.*)<\/a>~Ui';
    const CINEMACITY_SEASONS       = '~episodes\?season=(?:\d+)">(\d+)<~Ui';
    const CINEMACITY_SOUND_MIX     = '~<h5>Sound Mix:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_TAGLINE       = '~<h5>Tagline:<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_TITLE         = '~property="og:title" content="(.*)"~Ui';
    const CINEMACITY_TITLE_ORIG    = '~<span class="title-extra">(.*) <i>\(original title\)<\/i></span>~Ui';
    const CINEMACITY_TRAILER       = '~data-video="(.*)"~Ui';
    const CINEMACITY_URL           = '~http://(?:.*\.|.*)CINEMACITY.com/(?:t|T)itle(?:\?|/)(..\d+)~i';
    const CINEMACITY_USER_REVIEW   = '~<h5>User Reviews:<\/h5>(?:\s*)<div class="info-content">(.*)<a~Ui';
    const CINEMACITY_VOTES         = '~<a href="ratings" class="tn15more">(.*) votes<\/a>~Ui';
    const CINEMACITY_WRITER        = '~<h5>(?:Writer|Writers):<\/h5>(?:\s*)<div class="info-content">(.*)<\/div>~Ui';
    const CINEMACITY_YEAR          = '~content="(?:.*)\(*(\d{4})\)~Ui';

    /**
     * @param string $sSearch    CINEMACITY URL or movie title to search for.
     * @param null   $iCache     Custom cache time in minutes.
     * @param string $sSearchFor What to search for?
     *
     * @throws \CINEMACITYException
     */
    public function __construct($sSearch='', $iCache = null, $sSearchFor = 'all') {
        ...
        if ( ! is_writable($this->sRoot . '/posters') && ! mkdir($this->sRoot . '/posters')) {
            throw new CINEMACITYException('The directory “' . $this->sRoot . '/posters” isn’t writable.');
        }
        */
        if ( ! function_exists('curl_init')) {
            throw new CINEMACITYException('You need to enable the PHP cURL extension.');
        }
        /*
    ...
    private function fetchUrl() {
        try {
            $content = file_get_contents('http://cinemacity.bg/');
    ...
    public function getAka() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_AKA, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getAkas() {
        if (true === $this->isReady) {
            ...
            if ($bUseCache) {
                ...
                return CINEMACITYHelper::arrayOutput($this->bArrayOutput, $this->sSeparator, self::$sNotFound, $aReturn);
            } else {
                $fullAkas  = sprintf('http://www.CINEMACITY.com/title/tt%s/releaseinfo', $this->iId);
                $aCurlInfo = CINEMACITYHelper::runCurl($fullAkas);
                ...
                $aReturned = CINEMACITYHelper::matchRegex($sSource, "~<td>(.*?)<\/td>\s+<td>(.*?)<\/td>~");

                if ($aReturned) {
                    $aReturn = array();
                    foreach ($aReturned[1] as $i => $strName) {
                        if (strpos($strName, '(') === false) {
                            $aReturn[] = array('title'   => CINEMACITYHelper::cleanString($aReturned[2][$i]),
                                               'country' => CINEMACITYHelper::cleanString($strName));
                        }
                    }
        ...
        return CINEMACITYHelper::arrayOutput($this->bArrayOutput, $this->sSeparator, self::$sNotFound);
    }
    ...
    public function getAspectRatio() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_ASPECT_RATIO, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getAwards() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_AWARDS, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getCast($iLimit = 0, $bMore = true) {
        if (true === $this->isReady) {
            $aMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_CAST);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    if (0 !== $iLimit && $i >= $iLimit) {
                        break;
                    }
                    $aReturn[] = CINEMACITYHelper::cleanString($sName);
                }
                ...
                return CINEMACITYHelper::arrayOutput($this->bArrayOutput, $this->sSeparator, self::$sNotFound, $aReturn, $bHaveMore);
            }
        }
        ...
        if (true === $this->isReady) {
            ...
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    ...
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/name/' . CINEMACITYHelper::cleanString($aMatch[1][$i]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a>';
                }
    ...
    public function getCastAndCharacter($iLimit = 0, $bMore = true) {
        if (true === $this->isReady) {
            $aMatch     = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_CAST);
            $aMatchChar = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_CHAR);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    if (0 !== $iLimit && $i >= $iLimit) {
                        break;
                    }
                    $aReturn[] = CINEMACITYHelper::cleanString($sName) . ' as ' . CINEMACITYHelper::cleanString($aMatchChar[1][$i]);
                }
        ...
        if (true === $this->isReady) {
            ...
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    ...
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/name/' . CINEMACITYHelper::cleanString($aMatch[1][$i]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a> as ' . CINEMACITYHelper::cleanString($aMatchChar[1][$i]);
                }
    ...
    public function getCertification() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_CERTIFICATION, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getColor() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_COLOR, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getCompanyAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $aMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_COMPANY);
            if (isset($aMatch[2][0])) {
                return '<a href="http://www.CINEMACITY.com/company/' . CINEMACITYHelper::cleanString($aMatch[1][0]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($aMatch[2][0]) . '</a>';
            }
        }
    ...
    public function getCountryAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $aMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_COUNTRY);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/country/' . trim($aMatch[1][$i]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a>';
                }
    ...
    public function getCreatorAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_CREATOR, 1);
            $aMatch = CINEMACITYHelper::matchRegex($sMatch, self::CINEMACITY_NAME);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/name/' . CINEMACITYHelper::cleanString($aMatch[1][$i]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a>';
                }
    ...
    public function getDirectorAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_DIRECTOR, 1);
            $aMatch = CINEMACITYHelper::matchRegex($sMatch, self::CINEMACITY_NAME);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/name/' . CINEMACITYHelper::cleanString($aMatch[1][$i]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a>';
    ...
    public function getGenreAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $aMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_GENRE);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/Sections/Genres/' . CINEMACITYHelper::cleanString($aMatch[1][$i]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a>';
                }
    ...
    public function getLanguageAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $aMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_LANGUAGE);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/language/' . CINEMACITYHelper::cleanString($aMatch[1][$i]) . '"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a>';
                }
    ...
    public function getLocationAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $aMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_LOCATION);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/search/title?locations=' . CINEMACITYHelper::cleanString($aMatch[1][$i]) . '"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a>';
                }
    ...
    public function getMpaa() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_MPAA, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getPlotKeywords() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_PLOT_KEYWORDS, 1);
            if (false !== $sMatch) {
                $aReturn = explode('|', CINEMACITYHelper::cleanString($sMatch));
    ...
    public function getPlot($iLimit = 0) {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_PLOT, 1);
            if (false !== $sMatch) {
                if ($iLimit !== 0) {
                    return CINEMACITYHelper::getShortText(CINEMACITYHelper::cleanString($sMatch), $iLimit);
                }
    ...
    public function getPoster($sSize = 'small', $bDownload = false) {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_POSTER, 1);
            if (false !== $sMatch) {
                ...
                if (false === $bDownload) {
                    ...
                    $sLocal = CINEMACITYHelper::saveImage($sMatch, $this->iId);
    ...
    public function getRating() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_RATING, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getReleaseDate() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_RELEASE_DATE, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getRuntime() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_RUNTIME, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getSeasonsAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $aMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_SEASONS);
            if (count($aMatch[1])) {
                foreach ($aMatch[1] as $i => $sName) {
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/title/tt' . $this->iId . '/episodes?season=' . $sName . '"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . $sName . '</a>';
                }
    ...
    public function getSoundMix() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_SOUND_MIX, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getTagline() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_TAGLINE, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getTitle($bForceLocal = false) {
        if (true === $this->isReady) {
            if (true === $bForceLocal) {
                $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_TITLE_ORIG, 1);
                if (false !== $sMatch && "" !== $sMatch) {
                    return CINEMACITYHelper::cleanString($sMatch);
                }
            }

            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_TITLE, 1);
    ...
    public function getTrailerAsUrl($bEmbed = false) {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_TRAILER, 1);
            if (false !== $sMatch) {
                $sUrl = 'http://www.CINEMACITY.com/video/CINEMACITY/' . $sMatch . '/' . ($bEmbed ? 'player' : '');

                return CINEMACITYHelper::cleanString($sUrl);
            }
        }
    ...
    /**
     * @return string The CINEMACITY URL.
     */
    public function getUrl() {
        if (true === $this->isReady) {
            return CINEMACITYHelper::cleanString(str_replace('combined', '', $this->sUrl));
        }
    ...
    public function getUserReview() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_USER_REVIEW, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getVotes() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_VOTES, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
    ...
    public function getWriterAsUrl($sTarget = '') {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_WRITER, 1);
            $aMatch = CINEMACITYHelper::matchRegex($sMatch, self::CINEMACITY_NAME);
            if (count($aMatch[2])) {
                foreach ($aMatch[2] as $i => $sName) {
                    $aReturn[] = '<a href="http://www.CINEMACITY.com/name/' . CINEMACITYHelper::cleanString($aMatch[1][$i]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>' . CINEMACITYHelper::cleanString($sName) . '</a>';
    ...
    public function getYear() {
        if (true === $this->isReady) {
            $sMatch = CINEMACITYHelper::matchRegex($this->sSource, self::CINEMACITY_YEAR, 1);
            if (false !== $sMatch) {
                return CINEMACITYHelper::cleanString($sMatch);
            }
        }
...
class CINEMACITYHelper extends CINEMACITY
{
    /**
    ...
    public static function runCurl($sUrl, $bDownload = false) {
        ...
        curl_setopt_array($oCurl, array(CURLOPT_BINARYTRANSFER => ($bDownload ? true : false),
                                        CURLOPT_CONNECTTIMEOUT => self::CINEMACITY_TIMEOUT,
                                        CURLOPT_ENCODING       => '',
                                        CURLOPT_FOLLOWLOCATION => false,
                                        CURLOPT_FRESH_CONNECT  => true,
                                        CURLOPT_HEADER         => ($bDownload ? false : true),
                                        CURLOPT_HTTPHEADER     => array('Accept-Language:' . self::CINEMACITY_LANG,
                                                                        'Accept-Charset:' . 'utf-8, iso-8859-1;q=0.8',),
                                        CURLOPT_REFERER        => 'http://www.google.com',
                                        CURLOPT_RETURNTRANSFER => true,
                                        CURLOPT_TIMEOUT        => self::CINEMACITY_TIMEOUT,
...
//        return $aCurlInfo;
    ...
    public static function saveImage($sUrl, $iId) {
        if (preg_match('~title_addposter.jpg|CINEMACITY-share-logo.png~', $sUrl)) {
...
class CINEMACITYException extends Exception
{
}
